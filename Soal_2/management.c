...
void timelog(char *name, char *action){
    ...
    FILE *file = fopen("/home/purofuro/Fico/tes/history.log", "a"); 
    if (file != NULL) {  
        fprintf(file, "%s\n", result); 
        fclose(file); 
    } else { 
...
int ada(const char *fname){
    char fixname[100] = "/home/purofuro/Fico/tes/";
...
int default_act(){
    ...
    char *basePath = "/home/purofuro/Fico/tes/library";
    struct dirent *dp;
    ...
    {   
        ...
        if(default_mode == 1){
            if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0){
                ...
                char to_change[1000] = "/home/purofuro/Fico/tes/library/";
                char original_name[1000] = "/home/purofuro/Fico/tes/library/";
                char text[100], ch;
                int key = 19;
                ...
                if(text[0] >= '0' && text[0] <= '9'){
                    ...
                    if(strstr(dp->d_name, "d3Let3") != NULL){
                        ...
                        if(strstr(text, ".ts") != NULL){
                            rename(to_change, "/home/purofuro/Fico/tes/library/helper.ts");
                            timelog("helper.ts", "rename");
                        }
                        else if(strstr(text, ".py") != NULL){
                            rename(to_change, "/home/purofuro/Fico/tes/library/calculator.py");
                            timelog("calculator.py", "rename");
                        }
                        else if(strstr(text, ".go") != NULL){
                            rename(to_change, "/home/purofuro/Fico/tes/library/server.go");
                            timelog("server.go", "rename");
                        }
                        else{
                            rename(to_change, "/home/purofuro/Fico/tes/library/renamed.file");
...
int backup_act(){
    ...
    while ((dp = readdir(dir)) != NULL){
        ...
        char filename[1000] = "/home/purofuro/Fico/tes/library/";
        if(backup_mode == 1){
            if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0){
                ...
                if(strstr(dp->d_name, "m0V3") != NULL){
                    ...
                    if(multi_child == 0){
                        char *argv[4] = {"backing up", filename, "/home/purofuro/Fico/tes/library/backup", NULL}; 
                        execv("/bin/mv", argv);
                    }
                }
...
int restore_act(){
    ...
    int status;
    char *basePath = "/home/purofuro/Fico/tes/library/backup";
    struct dirent *dp;
    DIR *dir = opendir(basePath);
    ...
    while ((dp = readdir(dir)) != NULL){
        ...
        char filename[1000] = "/home/purofuro/Fico/tes/library/backup/";
        if(restore_mode == 1){
            if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0){
                ...
                if(strstr(dp->d_name, "m0V3") != NULL){
                    ...
                    pid_t multi_child = fork();
                    if(multi_child == 0){
                        char *argv[4] = {"backing up", filename, "/home/purofuro/Fico/tes/library", NULL}; 
                        execv("/bin/mv", argv);
                    }
                }
        ...
        if(argc == 1){
            ...
            if(ada("library.zip") == 0){
                ...
                if(download_child < 0){
                    ...
                    char *argv[6] = {"Download", "--content-disposition", "https://docs.google.com/uc?export=download&id=1rUIZmp10lXLtCIH3LAZJzRPeRks3Crup", "-P", "/home/purofuro/Fico/tes", NULL};
                    execv("/bin/wget", argv);
                    }
                }
            wait(&status);
            if(WIFEXITED(status)){
                if(ada("history.log") == 0){
                    ...
                    if(history_child == 0){
                        char *argv[3] = {"make a history", "/home/purofuro/Fico/tes/history.log", NULL};
            ...
            if(WIFEXITED(status)){
                if(ada("library") == 0){
                    ...
                    if(unzip_child == 0){
                        char *argv[5] = {"unzip", "/home/purofuro/Fico/tes/library.zip", "-d", "/home/purofuro/Fico/tes", NULL};
            ...
            wait(&status);
            if(WIFEXITED(status)){
                if(ada("library/backup") == 0){
                    pid_t mkdir_child = fork();
                    if(mkdir_child == 0){
                        char *argv[3] = {"making folder", "/home/purofuro/Fico/tes/library/backup", NULL};
